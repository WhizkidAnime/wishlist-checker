...
export const ManageShareLinksModal: React.FC<ManageShareLinksModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      ...
      role="dialog"
    >
      <div className="w-full max-w-2xl rounded-2xl shadow-xl p-4 sm:p-6 mx-auto flex flex-col max-h-[85vh] overflow-hidden" style={{ backgroundColor: 'var(--color-card-background)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          {loading ? (
            ...
            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
              {links.map((l) => (
                <li key={l.id} className="px-2 sm:px-3 py-2">
                  <div className="group flex items-center gap-3 justify-between hover:bg-theme-hover rounded-lg px-2 sm:px-3 py-2 transition-colors">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <a href={l.url} target="_blank" rel="noreferrer" className="text-xs text-blue-600 dark:text-blue-400 break-all hover:underline">{l.url}</a>
                      </div>
                    </div>
                    <div className="flex gap-2 ml-auto">
                      <button onClick={() => handleCopy(l.url, l.id)} className="h-9 px-3 rounded-full text-sm bg-theme-button text-theme-button hover:bg-theme-button transition-colors">
                        {copiedId === l.id ? 'Скопировано' : 'Копировать'}
                      </button>
                      <button onClick={() => openConfirm(l.id, l.title)} disabled={deletingId === l.id} className="h-9 px-3 rounded-full text-sm border border-red-300 dark:border-red-600 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50">
        ...
        <div className="mt-4 flex justify-end">
          <button onClick={onClose} className="h-10 px-4 rounded-full text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200">Закрыть</button>
        </div>
